...
- 408 backend tests at 90% line coverage, plus 140 frontend tests.
...
| 21–22 | Intelligence and dashboards | **PARTIAL** | Aggregate counts over the platform's own records — what a body has evidenced, been asked and done. **Every figure carries the basis that produced it**, and `/intelligence/records` resolves that basis back to the rows; a test walks every figure in the overview and requires its own drill-down to reconcile. Breakdowns of **citizen-submitted** records suppress any bucket below five, **plus a complement** so the residual cannot be recovered by subtraction. Dimensions are a closed set, and a test asserts none of them groups people rather than records. There is a **dashboard**: headline figures as stat tiles and four breakdowns, where every number links to the records it counted and a withheld bucket says `Fewer than 5 records` rather than rendering as a blank. Breakdowns are tables rather than bar charts because a suppressed bucket cannot be drawn as a bar honestly — zero length would say it is zero, omission would say it does not exist — and bars scale to the largest *reported* bucket so a withheld one cannot be measured off the others. **Not built:** time series (every figure is a count as of now), saved intelligence reports, cross-organisation comparison, narrowing the figures to one organisation or one area, breakdowns by area or theme (their buckets are identifiers, and a table of UUIDs answers nothing), and any caching. See [INTELLIGENCE.md](INTELLIGENCE.md). |
...
suppression actually fires: all nine headline figures were read off the
rendered page, each link followed, and every drill-down total agreed with the
number that led to it. A withheld bucket, a withheld headline figure, a
`(not recorded)` bucket resolving through an IS NULL basis, and a refused
measure were each looked at on screen rather than assumed from the tests.

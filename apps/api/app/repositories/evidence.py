...
class EvidenceRepository(BaseRepository[Evidence]):
    ...
    def get_by_status(
        self, organisation_id: str, status: EvidenceStatus, skip: int = 0, limit: int = 100
    ) -> tuple[List[Evidence], int]:
    ...
    def get_by_thematic_area(
        self, thematic_area_id: str, skip: int = 0, limit: int = 100
    ) -> tuple[List[Evidence], int]:
    ...
    def mark_approved(self, evidence_id: str, approved_by: str) -> Optional[Evidence]:
    ...
    def publish(self, evidence_id: str) -> Optional[Evidence]:
        """Publish evidence."""
        evidence = self.get_by_id(evidence_id)
